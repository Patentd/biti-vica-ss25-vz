...
resource "exoscale_compute_instance" "vm" {
  ...
  # Local-exec provisioner calls Ansible
  provisioner "local-exec" {
    command = <<EOT
      sleep 20;
      ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -i "${self.public_ip_address}," -u ubuntu --private-key "${path.module}/id_rsa" ${path.module}/ansible/playbook.yaml
    EOT
  }
